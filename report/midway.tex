...
Our previous plan was to generate a pandora-radio-like playlist based on a user input €œ€video, where the list was supposed to appeal to a viewer who enjoyed the seed video.  However, this task poses several challenges in evaluation metrics, which are not trivial given our current available sources such as time, efforts, etc. First, viewer opinion is highly subjective and diverse. Hence, to make an objective judgment on effectiveness of our algorithms, we do require a significant amount of user study. Two possible solutions are 1) running a real services and record user's feedback on the system and 2) using Amazon Mechanical Turk to evaluate the result. Both solutions are either costly or time-consuming, and not easy to achieve within our remaining time to final submission. 
